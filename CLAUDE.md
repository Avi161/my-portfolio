...
## Blog posts
- **Public** posts live in `src/data/posts.json` as an array of `{ slug, title, date, summary, content, category?, tags?, listed?, public? }`, newest first. `src/data/blogPosts.js` is a thin shim that imports it (sorted by date desc) — page imports go through the shim.
- `category` is a slash-separated **section path** ("Journal/2026/July", ≤80 chars total, ≤40/segment; top-level "Private" reserved). `/blog` renders top-level sections as tabs plus one nested chip row per level (path helpers in `src/lib/sections.js`; years/months auto-sort newest-first via `smartCompare`). `tags` is an array (≤12, ≤30 chars each, deduped case-insensitively server-side); entries/post pages show `#tag` links, `?tag=` filters and combines with `?section=`.
- `listed: false` hides a post from the default /blog list — it's reachable only via its section tab. A section whose posts are ALL unlisted renders as a dashed aside tab (this **generalizes the old hard-coded Miscellaneous behavior** — no special-case left in code). A hidden post must have a section (client guard in PostEditor).
- **Private tab on /blog**: dashed aside tab, always visible. Logged out it shows ONLY "This section is locked." + a password form (owner's choice: no titles/counts leak) — submitting calls `/api/login`, sets the same session as /admin, then private posts render under virtual `Private/...` paths with their own section rows; a "Lock" button clears the token. Private posts no longer appear in any other view (they used to mix into All when logged in).
...
- Editor meta controls: **SectionPicker** (`src/pages/admin/SectionPicker.js`, cascading selects over path segments with "+ New…" per level), **TagsInput** (`TagsInput.js`, chip input, Enter/comma commits, datalist suggestions), Visibility Public/Private, and a **Main page Shown/Hidden** toggle (public posts only → `listed`). Contract tests: `.scratch/test-sections-tags.js`.
- Required env vars (Vercel dashboard, Production): `ADMIN_PASSWORD`, `GITHUB_TOKEN` (fine-grained PAT, Contents R/W on this repo AND `my-portfolio-private`). Local `.env` is gitignored; CRA's `npm start` can't serve `/api` — use `vercel dev` or `.scratch/admin-test-server.js` (mock GitHub; `E2E_ADMIN_PASSWORD=...` overrides the password for automated runs so the real `.env` is never touched).
...
### [2026-07-15] E2E auth without touching the real .env [WORKS]
- Never overwrite/edit the user's `.env` for a test run, even "temporarily with a backup" (the permission classifier blocks it, and rightly so). Instead the mock server honors `E2E_ADMIN_PASSWORD` (checked before the `.env` read in `loadAdminPassword`) — launch with `E2E_ADMIN_PASSWORD=<throwaway> node .scratch/admin-test-server.js` and type the throwaway in the browser.
- **[TRAP]** browser_batch focus does NOT persist across batches: an autofocused input blurs between calls, and blur handlers commit/cancel (SectionPicker reverted to its select with empty text). Always click the input and type within the SAME batch.
- **[TRAP]** `.scratch/test-private-posts.js` asserted no `'secret'` substring anywhere in the public repo — a legit post ("…the tests could stay secret…") false-positived. Sweep assertions must match the fixture's unique markers (`secret-post`), not common words.
...
- **Decision (wall-staring tracker):** the owner wanted a clean grid and is fine editing it in HTML mode, so the post body uses a real `<table class="habit-tracker">` (styled under `.blog-post-content .habit-tracker` in `App.css`). `richModeWouldAlter` flags it, so it **auto-opens in HTML mode** — edit the cells there. **Never switch this post to Rich/Markdown mode** or the grid flattens into one paragraph (cells concatenated). There's no table extension installed, so the warning banner is expected, not a bug.
- **Rule:** Only use `<table>` in a post if you accept HTML-mode-only editing for it. If you want a tracker that's *also* Rich-editable, use a schema-native `<ul><li><p><strong>Label</strong> — …</p></li></ul>` checklist instead (`ul`/`li`/`p`/`strong` are all in StarterKit; store list items as `<li><p>…</p></li>`, TipTap's canonical form, same as the `wias`/references posts, so `richModeWouldAlter` returns false). Adding real Rich table support would require `@tiptap/extension-table` (Table/Row/Header/Cell) in `EDITOR_EXTENSIONS` **and** storing canonical table HTML so the round-trip is stable — not currently done.
